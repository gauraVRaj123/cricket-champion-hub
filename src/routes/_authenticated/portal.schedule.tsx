...
type Batch = { id: string; batch_name: string; age_group: string; days: string; start_time: string; end_time: string; location: string | null; notes: string | null };
...
function MySchedule() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: s } = await supabase.from("students").select("batch_id").eq("user_id", uid).maybeSingle();
      const batchId = (s as { batch_id: string | null } | null)?.batch_id;
      if (batchId) {
        const { data: b } = await supabase.from("batch_schedules").select("*").eq("id", batchId).maybeSingle();
  ...
  return (
    <div>
      ...
      {!loading && !batch && <p className="text-sm text-muted-foreground">You're not assigned to a batch yet.</p>}
      {batch && (
        <div className="border border-border p-6 max-w-lg">
          <div className="font-mono text-[10px] uppercase tracking-[0.25em] text-primary">{batch.age_group}</div>
