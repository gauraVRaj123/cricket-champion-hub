...
function AdminFees() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    if (!form.student_id || !form.amount)
      return toast.error("Student and amount required");
  ...
  const paidThisMonth = new Set(
    payments
      .filter((p) => p.fee_month.slice(0, 7) === currentMonth.slice(0, 7))
      .map((p) => p.student_id),
  );
  const pending = students.filter(
    (s) => !paidThisMonth.has(s.id) && (s.monthly_fee ?? 0) > 0,
  );
        ...
        <form onSubmit={submit} className="space-y-3 border border-border p-6">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setForm({
                  ...form,
                  student_id: sid,
                  amount: st?.monthly_fee?.toString() ?? form.amount,
                });
              }}
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field
              ...
              onChange={(v) => setForm({ ...form, fee_month: v })}
            />
            <Field
              label="Paid on"
              type="date"
        ...
        <div className="space-y-6">
          <div>
            <h2 className="font-display text-2xl mb-3">
              Pending This Month ({pending.length})
            </h2>
            {pending.length === 0 ? (
              ...
              <ul className="space-y-1 text-sm">
                {pending.map((s) => (
                  <li
                    key={s.id}
                    className="border border-border p-2 flex justify-between"
                  >
                    <span>{s.name}</span>
