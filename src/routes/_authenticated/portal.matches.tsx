          ...
          return (
            <div key={m.id} className="border border-border p-5">
              <div className="flex flex-wrap justify-between gap-2">
                ...
                <span className="font-mono text-[10px] uppercase tracking-widest text-primary border border-primary px-2 py-1 self-start">
                  {m.format}
                </span>
              </div>
              {m.notes && (
                <p className="text-sm mt-2 text-muted-foreground">{m.notes}</p>
              )}
              {showPerf && p && (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4 pt-4 border-t border-border text-sm">
                  <Stat
                    label="Runs"
                    value={`${p.batting_runs} (${p.balls_faced})`}
                  />
                  <Stat label="4s / 6s" value={`${p.fours} / ${p.sixes}`} />
                  <Stat
                    label="Bowling"
                    value={`${p.wickets}/${p.runs_conceded} (${p.overs_bowled})`}
                  />
